...
class TD3Agent:
    ...
    def _td3_update_step(self, batch, do_actor_update: bool) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            next_act = self._to_tensor_action(out_tgt)                 # (B, act_dim)
            ...
            q1_tgt   = self.critic1_tgt(next_obs, next_act)            # (B,1)
            q2_tgt   = self.critic2_tgt(next_obs, next_act)            # (B,1)
            min_q    = torch.min(q1_tgt, q2_tgt)                       # (B,1)
            target_q = rewards + self.gamma * (1.0 - dones) * min_q    # (B,1)
    
        # ----- Critic update -----
        current_q1   = self.critic1(obs, actions)                      # (B,1)
        current_q2   = self.critic2(obs, actions)                      # (B,1)
        ...
        if do_actor_update:
            out = self.actor(obs)
            pi  = self._to_tensor_action(out)                          # (B, act_dim)
